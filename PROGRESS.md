...
| 测试 | ✅ **169/169 通过** | 11 个测试文件全绿 |
| 本地代理 | ✅ 已接入 | 搜索覆盖全市场，非白名单域名被拒 |
...
### 2026-09-15（AI 实施 · 第八轮）本地代理：搜索覆盖全市场

**背景**：第七轮发现腾讯搜索接口 `smartbox.gtimg.cn` 无 CORS 头，
浏览器直接拦死（`net::ERR_FAILED`）。当时的妥协是内置 85 支股票做本地搜索，
但用户搜「招商」只能出一支（招商银行），而实际有十支。

**方案**：Electron 主进程内起一个本地 HTTP 代理。
请求由 Node 侧发出（不受同源策略约束），再把结果转给前端。

**1. 代理服务（`src/main/proxy.ts`）**

- 只监听 `127.0.0.1`，不对外暴露
- **域名白名单**：仅允许转发到 `smartbox.gtimg.cn` / `qt.gtimg.cn` / `web.ifzq.gtimg.cn`，
  且必须是 https。**没有这道检查，本地代理就成了任意网站的 SSRF 跳板**
- 端口占用时自动顺延（默认 5198 起）
- 上游失败返回 502，不崩溃
- fetch 可注入，便于测试

**2. 主进程集成（`src/main/index.ts`）**

先起代理再开窗口，并通过 IPC (`get-proxy-port`) 把端口告知渲染层。
代理启动失败**不影响主功能**——行情接口本身带 CORS，只是搜索退回内置表。

**3. 渲染层客户端（`src/renderer/services/sources/proxyClient.ts`）**

- Electron 下经 `ipcRenderer` 拿端口
- 纯浏览器（开发预览）下探测 5198，失败即视为无代理
- 代理不可用时返回 null 而非抛异常，调用方据此降级
...
`src/main/proxy.test.ts` 重点覆盖**安全边界**而非转发本身：
缺失/非法 url → 400、非白名单域名 → 403、file 协议 → 403、
非 GET → 405、OPTIONS 预检 → 204、上游失败 → 502、端口占用顺延。

**6. 端到端验证（真实 Electron + 真实网络）**

```
[StockAll] 本地行情代理已启动: http://127.0.0.1:5198
IPC端口=5198 | proxyClient=5198

招商 -> 10 条: 600036/招商银行, 601872/招商轮船, 600999/招商证券, 001979/招商蛇口…
宁德 ->  2 条: 300750/宁德时代, 03750/宁德时代
贵州 ->  4 条: 600519/贵州茅台, 600903/贵州燃气, 000589/贵州轮胎, 06199/贵州银行
```

对比第七轮（本地表只有 85 支）：「招商」当时仅 1 条，现在 10 条。

**门禁**：TypeScript 0 错误 | ESLint 0 错误 | 测试 **169/169** | 构建通过
